...
- `auth-ui.js::handleSignup(...)` — el `sendEmailVerification(result.user).catch(() => {})` agregado en la limpieza de auth (best-effort, tras crear la cuenta) también pasa por el mismo SDK cliente roto. Se reemplaza por una llamada a `POST /api/me/verification-email` usando el `idToken` recién obtenido de `result.user.getIdToken()` (mismo token que ya se usa para `/api/auth/sync` un par de líneas arriba) — sigue siendo best-effort/no bloqueante, solo cambia el transporte.
