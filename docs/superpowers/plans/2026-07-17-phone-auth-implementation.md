...
- **CSP exists in THREE places, not one — all three govern `auth.html` depending on environment, and all three need the `https://www.google.com` addition (found during plan review, not in the original spec):**
  ...
  `auth.html` and `vercel.json` currently have byte-identical CSP strings; `api/index.js`'s is narrower (no Firebase-specific entries, and no `frame-src` directive at all, since it's dev/test-only). Task 9 adds only the `www.google.com` entries (plus a new `frame-src` for `api/index.js`) appropriate to each file's own existing baseline — it does NOT make all three byte-identical to each other, just each internally consistent with what it already had.
...
- `auth.html` and `vercel.json` currently have byte-identical CSP strings; `api/index.js`'s is narrower (see Global Constraints). This task adds the `www.google.com` entries appropriate to each file's own baseline, not a uniform copy across all three.
